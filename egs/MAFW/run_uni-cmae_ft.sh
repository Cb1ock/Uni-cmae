...
model=uni-cmae-ft # or uni_cmae_ablation
...
pretrain_path="/home/hao/Project/uni-cmae/pretrained_model/UniCmae_uninorm_bs50_PixnormTrue_tpTrue_c0.01_0.9_0.5_1e-4_16frame.pth"
...
do
    ...
    exp_dir=./exp/new-weights_1_full-${model}-${balance}-${lr}-${optimizer}-bs${batch_size}--${ftmode}-mx${mixup}-dp${drop_path}-fz${freeze_base}-h${head_lr}-war/fold${fold}
    mkdir -p $exp_dir

    CUDA_CACHE_DISABLE=1 CUDA_VISIBLE_DEVICES=1,2,4,6 python -W ignore ../../src/run_unicmae_ft.py --model ${model} --dataset ${dataset} \
