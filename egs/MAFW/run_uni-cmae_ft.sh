...
do
    ...
    exp_dir=./exp/full-${model}-${balance}-${lr}-${lrscheduler_start}-${lrscheduler_decay}-${lrscheduler_step}-bs${batch_size}--${ftmode}-fz${freeze_base}-h${head_lr}-war/fold${fold}
    mkdir -p $exp_dir
    ...
    --label-csv ${label_csv} --n_class 11 --num_tests ${num_tests} \
