...
class cav_mae(nn.Module):
    ...
    def __init__(self, img_size=224, audio_length=1024, patch_size=16, in_chans=3,
        ...
        # Main ViT Block
        self.blocks_v = nn.ModuleList([Block(embed_dim, num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer, drop_path=drop_path) for i in range(encoder_depth-fusion_depth)])
        self.blocks_a = nn.ModuleList([Block(embed_dim, num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer, drop_path=drop_path) for i in range(encoder_depth-fusion_depth)])
